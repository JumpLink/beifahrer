 ...
 *
 * COPIED VERBATIM from troedler (`projects/troedler/app/src/frontends/mcp/runtime.ts`), itself a
 * verbatim copy of postbote's. **This is the THIRD copy.** troedler's header already said the
 * second copy was where to lift it into a shared `@gjsify/mcp`; that has not happened yet
 * (a new npm name needs a manual first publish). Until it does, a change here must be made in all
 * three copies or in none — drift would fail in one consumer while the others stay green.
 *
 * Keep it beifahrer-free — no import from anywhere else in this repo — so it can still move
 * verbatim.
 *
